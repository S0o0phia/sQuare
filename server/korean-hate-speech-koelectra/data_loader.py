...
class KoreanHateSpeechProcessor(object):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines[1:]):  # Except the header
            ...
            bias_label = None
            hate_label = None
            if set_type != "test" and set_type != "sQuare":
                bias_label = line[1]
                hate_label = line[2]
            '''
            if i % 1000 == 0:
    ...
    def get_examples(self, mode):
        ...
        file_to_read = None
        if mode == 'train':
            file_to_read = os.path.join(self.args.data_dir, self.args.train_file)
        elif mode == 'dev':
            file_to_read = os.path.join(self.args.data_dir, self.args.dev_file)
        elif mode == 'test':
            file_to_read = os.path.join(self.args.data_dir, self.args.test_file)
        elif mode == 'sQuare':
            file_to_read = self.args.sQ_tfile
...
#    logger.info("Creating features from dataset file at %s", args.data_dir)
    if mode == "train":
        ...
        examples = processor.get_examples("sQuare")
        '''
        examples = processor._create_examples(['title\tcomment','ㅇㅅㅎ 짱짱걸"\t"지랄하네 발닦고 자라 ㅋㅋ"\n',
        '페미년들\t왜사냐', '시발 미쳤나\t씨발ㅋㅋㅋ', '좆같은 페미니즘\t다죽이고싶다'], 'sQuare')
        
        contentLines = []
        post = ['title\tcomment']

        #title = args.sQ_title.encode('CP949').decode('CP949')
        #contentLines = args.sQ_content.encode('CP949').decode('CP949').split('\n')
        
        #title = args.sQ_title.replace('\r', '')
        title = args.sQ_title.decode('cp949', 'ignore')
        #contentLines = args.sQ_content.replace('\r', '').split('\n')
        contentLines = args.sQ_content.decode('cp949', 'ignore').split('\n')
        for line in contentLines:
            post.append(title + '\t' + line)
        examples = processor._create_examples(post, 'sQuare')
        '''
    else:
        raise Exception("For mode, Only train, dev, test is available")
    logger.info(args.max_seq_len)
