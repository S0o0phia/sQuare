...
class Trainer(object):
    def __init__(self, args, tokenizer, train_dataset=None, dev_dataset=None, test_dataset=None, sQuare_data=None):
        self.args = args
    ...
    def sQuare(self):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            if bias_preds is None:
                ...
                bias_preds = np.append(
                    bias_preds, bias_logits.detach().cpu().numpy(), axis=0)

            # Hate
            if hate_preds is None:
                hate_preds = hate_logits.detach().cpu().numpy()
            else:
                hate_preds = np.append(
                    hate_preds, hate_logits.detach().cpu().numpy(), axis=0)
...
#        logger.info("Saving model checkpoint to %s", self.args.model_dir)
    ...
    def load_model(self):
        ...
        if not os.path.exists(self.args.model_dir):
            raise Exception("Model doesn't exists! Train first! : " + self.args.model_dir)
